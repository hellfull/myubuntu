...
echo "[info] Setting up user credentials."
service mysql start $ sleep 10

echo "[info] Setting new root password."
...
echo "[info] Setting root remote password."
...
if [ -n "$MYSQL_USER_DB" ]; then
	echo "[info] Creating datebase from variable: $MYSQL_USER_DB"
	...
	if [ -n "$MYSQL_USER" ] && [ -n "$MYSQL_USER_PWD" ]; then
		echo "[info] Create new User: $MYSQL_USER with password $MYSQL_USER_PWD for new database $MYSQL_USER_DB."
	...
	if [ -n "$MYSQL_USER" ] && [ -n "$MYSQL_USER_PWD" ]; then
		echo "[info] Create a new User: $MYSQL_USER with password $MYSQL_USER_PWD for all database."
